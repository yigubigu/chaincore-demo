...
class Assets {
	public static void main(String[] args) throws Exception {
		Client client = new Client(Constant.CLIENT_URL,
		...
		new Account.Builder().setAlias("acme_treasury")
				.addRootXpub(accountKey.xpub).setQuorum(1).create(client);

		// snippet create-asset-acme-common
		new Asset.Builder().setAlias("acme_common").addRootXpub(assetKey.xpub)
				.setQuorum(1).addTag("internal_rating", "1")
				.addDefinitionField("issuer", "Acme Inc.")
				.addDefinitionField("type", "security")
				.addDefinitionField("subtype", "private")
				.addDefinitionField("class", "common").create(client);
		// endsnippet

		// snippet create-asset-acme-preferred
		new Asset.Builder().setAlias("acme_preferred")
				.addRootXpub(assetKey.xpub).setQuorum(1)
				.addTag("internal_rating", "2")
				.addDefinitionField("issuer", "Acme Inc.")
				.addDefinitionField("type", "security")
				.addDefinitionField("subtype", "private")
				.addDefinitionField("class", "preferred").create(client);
		// endsnippet

		// snippet list-local-assets
		Asset.Items localAssets = new Asset.QueryBuilder()
				.setFilter("is_local=$1").addFilterParameter("yes")
				.execute(client);
		...
		Asset.Items preferred = new Asset.QueryBuilder()
				.setFilter(
						"definition.type=$1 AND definition.subtype=$2 AND definition.class=$3")
				.addFilterParameter("security").addFilterParameter("private")
				.addFilterParameter("preferred").execute(client);
		...
		Transaction.Template issuanceTransaction = new Transaction.Builder()
				.addAction(
						new Transaction.Action.Issue().setAssetAlias(
								"acme_common").setAmount(1000))
				.addAction(
						new Transaction.Action.ControlWithAccount()
								.setAccountAlias("acme_treasury")
								.setAssetAlias("acme_common").setAmount(1000))
				.build(client);
		// endsnippet

		// snippet sign-issue
		Transaction.Template signedIssuanceTransaction = HsmSigner
				.sign(issuanceTransaction);
		// endsnippet
		...
		Transaction.Template retirementTransaction = new Transaction.Builder()
				.addAction(
						new Transaction.Action.SpendFromAccount()
								.setAccountAlias("acme_treasury")
								.setAssetAlias("acme_common").setAmount(50))
				.addAction(
						new Transaction.Action.Retire().setAssetAlias(
								"acme_common").setAmount(50)).build(client);
		// endsnippet

		// snippet sign-retire
		Transaction.Template signedRetirementTransaction = HsmSigner
				.sign(retirementTransaction);
		// endsnippet
		...
		Transaction.Items acmeCommonIssuances = new Transaction.QueryBuilder()
				...
				.addFilterParameter("issue").addFilterParameter("acme_common")
				.execute(client);
		...
		Transaction.Items acmeCommonTransfers = new Transaction.QueryBuilder()
				.setFilter("inputs(type=$1 AND asset_alias=$2)")
				.addFilterParameter("spend").addFilterParameter("acme_common")
				.execute(client);
		...
		Transaction.Items acmeCommonRetirements = new Transaction.QueryBuilder()
				...
				.addFilterParameter("retire").addFilterParameter("acme_common")
				.execute(client);
